...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
     ...
     */
    @Query("SELECT p FROM Payment p WHERE p.status = 'COMPLETADO' AND p.fechaPago >= :since")
     ...
     */
    @Query("SELECT COALESCE(SUM(p.monto), 0) FROM Payment p WHERE p.status = 'COMPLETADO' AND p.fechaPago >= :since")
    List<Object> findTotalSalesAmountSince(@Param("since") LocalDateTime since);
    
    /**
     * Pagos recientes agrupados por día
     */
    @Query(value = "SELECT DATE(fecha_pago) as fecha, SUM(monto) as total FROM pagos_ez WHERE fecha_pago >= :since GROUP BY DATE(fecha_pago)", 
           nativeQuery = true)
